...
const  Chat=({setindexofseletedtopic})=>{
    ...
    const {  allStudents, profobjId, AdminobjId } = useContext(Profc)

    const classes = useStyles();
   

    useEffect(() => {
        let data =[...allStudents,{studentId:"Admin",studentobjId:AdminobjId}]
            ...
            { sortedValue!==[] ? sortedValue.map((student,i)=>(
                 <Paper elevation={3} key={i}  className={classes.paper} >
                    <Box display="flex">
                        ...
                        <Box my="auto" mr={3}>
                          <Link to={`/ChatBox/`+student.studentobjId}>
                          <Button
                                type="submit"
                                variant="contained"
                                color="primary"
