...
const ChatBox = (props) => {
  ...
  const { profobjId } = useContext(Profc) 
  ...
  const [msg, setmsg] = useState("")
   ...
   {            let dummyallchats = []
                await Axios.get(`http://localhost:5000/chat`).then((res)=>{
                        ...
                        let data = res.data.filter(chat=> ((chat.to===profobjId && chat.from===studentobjId) || chat.to===studentobjId && chat.from===profobjId))
                        dummyallchats = data
                }).catch(err=>{
                    alert(err)
                })
  ...
  useEffect(() => {
    ...
    let newroom  =  props.match.params.id + profobjId 
  ...
  socket.on("c1remote", (msg)=>{
     msg = {
            ...msg,
            dir:msg.from===profobjId ? "right" : "left"
     } 
     console.log(msg);
    setchats([...chats, msg])
})
            ...
            <Grid item xs={12}>
                <List className={classes.messageArea}>
                    { chats.map((obj, i)=>(
                             ...
                             <Grid container>
                                 <Grid item xs={12}>
                                     <ListItemText className={classes.listText} align={obj.from===profobjId?"right":"left"} primary={obj.message}></ListItemText>
                                 </Grid>
                                 <Grid item xs={12}>
                                     <ListItemText align={obj.from===profobjId?"right":"left"} secondary={obj.Date}></ListItemText>
                                 </Grid>
                             </Grid>
                             <Divider />
                         </ListItem>
