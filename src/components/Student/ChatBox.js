...
const ChatBox = (props) => {
  ...
  const { studentobjId } = useContext(Stuc) 
  const [chats, setchats] = useState([])
  const [msg, setmsg] = useState("")
 
  
 
   async function fetchchat(profId)
   {            let dummyallchats = []
                await Axios.get(`http://localhost:5000/chat`).then((res)=>{

                        let data = res.data.filter(chat=> ((chat.to===profId && chat.from===studentobjId) || chat.to===studentobjId && chat.from===profId))
                        dummyallchats = data
                }).catch(err=>{
                    alert(err)
                })
  ...
  useEffect(() => {
    ...
    let newroom  = studentobjId + props.match.params.id 
  ...
  socket.on("c1remote", (msg)=>{
     msg = {
            ...msg,
            dir:msg.from===studentobjId ? "right" : "left"
     } 
     console.log(msg);
    setchats([...chats, msg])
})
            ...
            <Grid item xs={12}>
                <List className={classes.messageArea}>
                    { chats.map((obj, i)=>(
                             ...
                             <Grid container>
                                 <Grid item xs={12}>
                                     <ListItemText align={obj.from===studentobjId?"right":"left"} primary={obj.message}></ListItemText>
                                 </Grid>
                                 <Grid item xs={12}>
                                     <ListItemText align={obj.from===studentobjId?"right":"left"} secondary={obj.Date}></ListItemText>
                                 </Grid>
                             </Grid>
                             <Divider />
                         </ListItem>
